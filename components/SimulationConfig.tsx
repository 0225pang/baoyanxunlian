...
type SimulationStep = { id: string; title: string; kind: 'intro' | 'question' | 'fixed' | 'dynamic'; typeCode?: string; count?: number; timeSeconds?: number; allowFollowup?: boolean; prompt?: string };
...
const DEFAULT_DYNAMIC_QUESTION_PROMPT = '你是一名食品专业保研面试老师。请根据学员在本场面试此前完成的自我介绍、科研项目介绍及其他回答，设计一条适合“自由交流”环节的具体问题。优先追问其研究经历、项目细节、方法选择、结果意义或与食品专业的关联；若信息不足，可提出一条能了解研究兴趣与学术潜力的问题。只输出一条自然的面试问题，不要解释。';
...
export default function SimulationConfig() {
  ...
  function removeModule(index: number) {
    if (modules.length <= 1) { setMessage('模拟流程至少需要保留一个模块。'); return; }
    updateTemplate({ modules: modules.filter((_item, position) => position !== index) });
  }
  function addModule(kind: SimulationStep['kind']) {
    ...
    const item: SimulationStep = kind === 'question'
      ...
      : kind === 'fixed'
        ...
        : kind === 'dynamic'
          ? { id, title: '自由交流', kind, prompt: DEFAULT_DYNAMIC_QUESTION_PROMPT, count: 1, timeSeconds: 180, allowFollowup: true }
  ...
  async function removeTemplate() {
    if (!selected) return;
    if (!selected.id) { setSelected(templates[0] || null); return; }
    if (!window.confirm(`确定删除“${selected.name}”吗？已完成的真实模拟记录会继续保留。`)) return;
    setSaving(true); setMessage('');
    try { await requestJson('/api/simulations/config', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ deleteTemplateId: selected.id }) }); setMessage('模拟流程已删除，历史记录已保留。'); await load(); }
    catch (error) { setMessage((error as Error).message); } finally { setSaving(false); }
  }
