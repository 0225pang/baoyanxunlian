...
type Notification = { id: number; kind: 'success' | 'error' | 'info' | 'warning' | 'announcement'; title: string; content?: string | null; isRead: boolean; createdAt: string };
...
export default function NotificationBell() {
  ...
  const announcement = items.find((item) => item.kind === 'announcement' && !item.isRead);
  return <div ref={centerRef} className="notification-center"><button type="button" className="notification-bell" aria-label={`通知中心${unread ? `，${unread} 条未读` : ''}`} aria-expanded={open} onClick={toggle}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 9a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4" /></svg>{unread > 0 && <i>{unread > 99 ? '99+' : unread}</i>}</button>{open && <section className="notification-popover" role="dialog" aria-label="通知中心"><header><div><strong>通知中心</strong><small>打开后会自动标记普通通知为已读</small></div></header><div className="notification-list">{items.length ? items.map((item) => <article key={item.id} className={`notification-item ${item.kind} ${item.isRead ? 'read' : ''}`}><span aria-hidden="true" /> <div><b>{item.title}</b>{item.content && <p>{item.content}</p>}<time>{when(item.createdAt)}</time></div></article>) : <div className="notification-empty">还没有通知。完成转录、复盘、导入或配音后，结果会显示在这里。</div>}</div></section>}{announcement && <section className="announcement-modal" role="alertdialog" aria-modal="true" aria-label="系统公告"><div className="announcement-modal-mark" aria-hidden="true">公告</div><button className="announcement-modal-close" type="button" aria-label="关闭并标记已读" onClick={() => void readAnnouncement(announcement.id)}>×</button><p className="announcement-modal-kicker">SYSTEM UPDATE</p><h2>{announcement.title}</h2><p className="announcement-modal-content">{announcement.content}</p><time>{when(announcement.createdAt)}</time><button className="announcement-modal-confirm" type="button" onClick={() => void readAnnouncement(announcement.id)}>我已阅读</button></section>}</div>;
}
