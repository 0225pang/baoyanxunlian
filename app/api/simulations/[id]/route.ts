...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    for (let index = 0; index < answers.length; index += 1) {
      ...
      const questionAudio = form.get('question-audio-' + index);
      const questionAudioData = questionAudio instanceof File && questionAudio.size > 0 ? Buffer.from(await questionAudio.arrayBuffer()) : null;
