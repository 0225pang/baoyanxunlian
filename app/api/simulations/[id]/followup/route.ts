...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const payload = safeJsonParse(raw); const followup = extractChatContent(payload); if (!followup) return Response.json({ error: '未生成追问' }, { status: 502 });
    const usage = readTokenUsage(payload);
    await logApiUsage(Number(sessions[0].userId), 'ai', { inputTokens: usage.inputTokens || Math.ceil((prompt + String(body.question || '') + String(body.answer || '')).length / 2), outputTokens: usage.outputTokens || Math.ceil(followup.length / 2), model: config.model });
