...
export async function PATCH(request: Request) {
  try {
    ...
    const sambertApiKey = clean(value.sambertApiKey, 1000); const baiduApiKey = clean(value.baiduApiKey, 1000); const baiduSecretKey = clean(value.baiduSecretKey, 1000); const apiKey = clean(value.apiKey, 1000); const baiduTtsUrl = clean(value.baiduTtsUrl || current.baiduTtsUrl, 500) || 'https://tsn.baidu.com/text2audio'; const baiduCloneUrl = clean(value.baiduCloneUrl || current.baiduCloneUrl, 500) || 'https://aip.baidubce.com/rest/2.0/speech/publiccloudspeech/v1/voice/clone/create'; const baiduCloneTtsUrl = clean(value.baiduCloneTtsUrl || current.baiduCloneTtsUrl, 500) || 'https://aip.baidubce.com/rest/2.0/speech/publiccloudspeech/v1/voice/clone/tts';
    ...
    if (![cloneUrl, synthesisUrl, websocketUrl, sambertWebsocketUrl, publicBaseUrl, baiduTtsUrl, baiduCloneUrl, baiduCloneTtsUrl].every(validUrl)) return Response.json({ error: 'API 地址必须以 http(s):// 或 ws(s):// 开头。' }, { status: 400 });
    await execute(`UPDATE tts_settings SET provider=?,clone_url=?,synthesis_url=?,websocket_url=?,sambert_websocket_url=?,sambert_api_key=COALESCE(NULLIF(?,''),sambert_api_key),baidu_api_key=COALESCE(NULLIF(?,''),baidu_api_key),baidu_secret_key=COALESCE(NULLIF(?,''),baidu_secret_key),baidu_tts_url=?,baidu_clone_url=?,baidu_clone_tts_url=?,api_key=COALESCE(NULLIF(?,''),api_key),public_base_url=?,clone_model=?,clone_target_model=?,default_model=?,updated_at=CURRENT_TIMESTAMP WHERE id=1`, [provider, cloneUrl || null, synthesisUrl || null, websocketUrl || null, sambertWebsocketUrl || null, sambertApiKey, baiduApiKey, baiduSecretKey, baiduTtsUrl, baiduCloneUrl, baiduCloneTtsUrl, apiKey, publicBaseUrl || null, cloneModel, cloneTargetModel, defaultModel]);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await execute('INSERT INTO question_voices (question_id,name,kind,provider,status,model,voice_id,parameters,public_token) VALUES (?,?,?,?,?,?,?,?,?)', [isGlobalPreview ? null : questionId, name, kind, provider, 'processing', model, voiceId, JSON.stringify({ ...parameters, ...(isGlobalPreview ? { text } : {}) }), createPublicToken()]) as ResultSetHeader;
    const id = Number(result.insertId);
    try { const audio = await synthesizeVoice(settings, { provider, text: isGlobalPreview ? text : String(question.content), model, voiceId, parameters }); const extension = audio.mime === 'audio/wav' ? 'wav' : audio.mime === 'audio/pcm' ? 'pcm' : 'mp3'; const outputPath = await storeQuestionVoiceFile(id, `speech.${extension}`, audio.audio, 'output'); await execute('UPDATE question_voices SET status=?,output_path=?,output_mime=?,error=NULL WHERE id=?', ['ready', outputPath, audio.mime, id]); return Response.json({ id, state: await readState() }, { status: 201 }); }
