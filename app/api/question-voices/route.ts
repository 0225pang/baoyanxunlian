...
export async function POST(request: Request) {
  try {
    ...
    const provider = body.provider === 'baidu' ? 'baidu' : 'bailian'; const questionId = Number(body.questionId); const name = clean(body.name, 160) || '生成语音'; const settings = await getTtsSettings(); const model = provider === 'baidu' ? clean(body.model || 'baidu-duxiaoyu', 150) : clean(body.model || settings.defaultModel, 150); const voiceId = provider === 'baidu' ? clean(body.voiceId, 255) : model.startsWith('sambert-') ? '' : clean(body.voiceId, 255);
