...
export async function GET(request: Request) {
  try {
    ...
    const statsRows = await query<RowDataPacket[]>(`SELECT COUNT(*) AS totalSimulations, COALESCE(SUM(DATE(DATE_ADD(s.started_at, INTERVAL 8 HOUR)) = DATE(DATE_ADD(UTC_TIMESTAMP(), INTERVAL 8 HOUR))), 0) AS todaySimulations FROM simulation_sessions s WHERE s.status = 'completed'${user.role === 'admin' ? '' : ' AND s.user_id = ?'}`, user.role === 'admin' ? [] : [user.id]);
