...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const answers = await query<RowDataPacket[]>(`SELECT a.id, a.module_index AS moduleIndex, a.module_title AS moduleTitle, a.question_id AS questionId, a.question, a.answer, a.transcript, a.transcript_segments AS transcriptSegments, a.transcript_repair_count AS transcriptRepairCount, a.transcript_repair_status AS transcriptRepairStatus, a.transcript_repair_error AS transcriptRepairError, a.elapsed_seconds AS elapsedSeconds, a.followup_question AS followupQuestion, a.audio_data IS NOT NULL AS hasAudio, a.audio_mime AS audioMime, a.question_audio_data IS NOT NULL AS hasQuestionAudio, a.question_audio_mime AS questionAudioMime, q.answer AS referenceAnswer, q.subcategory, t.name AS questionTypeName, DATE_FORMAT(a.created_at, '%Y-%m-%dT%H:%i:%s') AS createdAt FROM simulation_answers a LEFT JOIN questions q ON q.id = a.question_id LEFT JOIN question_types t ON t.id = q.type_id WHERE a.session_id = ? ORDER BY a.module_index ASC, a.id ASC`, [id]);
