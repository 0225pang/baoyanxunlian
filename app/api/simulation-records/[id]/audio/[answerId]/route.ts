...
export async function GET(request: Request, context: { params: Promise<{ id: string; answerId: string }> }) {
  try {
    ...
    const rows = await query<RowDataPacket[]>(`SELECT a.audio_data AS audioData, a.audio_mime AS audioMime, a.question_audio_data AS questionAudioData, a.question_audio_mime AS questionAudioMime, s.user_id AS userId FROM simulation_answers a JOIN simulation_sessions s ON s.id=a.session_id WHERE a.id=? AND a.session_id=? LIMIT 1`, [answerId, sessionId]);
    const row = rows[0]; const audioData = kind === 'question' ? row?.questionAudioData : row?.audioData; const audioMime = kind === 'question' ? row?.questionAudioMime : row?.audioMime;
