...
export async function POST(request: Request) {
  ...
  if (user.status === 'disabled') return Response.json({ error: '该账号已被管理员禁用，暂时无法登录' }, { status: 403 });
  if (user.status !== 'active') {
    const error = user.status === 'pending' ? '注册申请正在等待管理员审核' : user.status === 'rejected' ? '注册申请未通过，请联系管理员' : '该账号已被删除';
    return Response.json({ error }, { status: 403 });
  }
  await createSession(user.id);
  await execute('UPDATE users SET last_login_at = CURRENT_TIMESTAMP, last_seen_at = CURRENT_TIMESTAMP WHERE id = ?', [user.id]);
