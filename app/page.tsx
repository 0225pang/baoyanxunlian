...
function formatAudioDuration(seconds: number | null) {
  if (seconds == null || !Number.isFinite(seconds) || seconds < 0) return '时长加载中…';
  const whole = Math.floor(seconds);
  return `录音时长 ${String(Math.floor(whole / 60)).padStart(2, '0')}:${String(whole % 60).padStart(2, '0')}`;
}

function AudioWithDuration({ src, className = '' }: { src: string; className?: string }) {
  const [duration, setDuration] = useState<number | null>(null);
  return <div className={'audio-with-duration ' + className}><audio controls preload="metadata" src={src} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onDurationChange={(event) => setDuration(event.currentTarget.duration)} /><small>{formatAudioDuration(duration)}</small></div>;
}
...
function Simulation({ onBack }: { onBack: () => void }) {
  ...
  async function next() { const audio = recording ? await stopRecording() : segmentBlob; if (current?.allowFollowup && (liveTranscript.trim() || answer.trim()) && !followup) { await beginFollowup(audio); return; } const saved = saveCurrent(undefined, undefined, undefined, audio); if (stepIndex + 1 < steps.length) { setStepIndex((value) => value + 1); setStepStartedAt(Date.now()); } else await finish(saved ? [...drafts, saved] : drafts); }
