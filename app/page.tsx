...
type SimulationRecord = { id: number; userId: number; templateName: string; status: string; totalSeconds: number; elapsedSeconds: number; startedAt: string; completedAt?: string | null; username?: string; displayName?: string; answerCount?: number; hasFullAudio?: number };
  ...
  return <div className="app">
    <header>
      ...
      <nav>
        ...
        <button className={page === 'simulation-history' ? 'active' : ''} onClick={() => setPage('simulation-history')}>真实模拟记录</button>
    ...
    {page === 'simulation-history' && <SimulationHistory onBack={() => setPage('home')} />}
...
type SimulationStep = { id: string; title: string; kind: 'intro' | 'question' | 'fixed' | 'dynamic'; typeCode?: string; count?: number; timeSeconds?: number; allowFollowup?: boolean; prompt?: string; templateModuleId?: string; questionId?: number; question?: string; category?: string; subcategory?: string | null; referenceAnswer?: string | null };
...
function Simulation({ onBack }: { onBack: () => void }) {
  const [templates, setTemplates] = useState<SimulationTemplate[]>([]); const [templateId, setTemplateId] = useState(''); const [sessionId, setSessionId] = useState(0); const [steps, setSteps] = useState<SimulationStep[]>([]); const [stepIndex, setStepIndex] = useState(0); const [answer, setAnswer] = useState(''); const [recording, setRecording] = useState(false); const [segmentBlob, setSegmentBlob] = useState<Blob | null>(null); const [drafts, setDrafts] = useState<SimulationAnswerDraft[]>([]); const [elapsed, setElapsed] = useState(0); const [stepStartedAt, setStepStartedAt] = useState(0); const [message, setMessage] = useState(''); const [followup, setFollowup] = useState<string | null>(null); const [followupGenerating, setFollowupGenerating] = useState(false); const [followupSourceAnswer, setFollowupSourceAnswer] = useState(''); const [fullAudio, setFullAudio] = useState<Blob | null>(null); const [autoRecord, setAutoRecord] = useState(true); const [readQuestion, setReadQuestion] = useState(false); const [countdown, setCountdown] = useState<number | null>(null); const [reading, setReading] = useState(false); const [promptCycle, setPromptCycle] = useState(0); const [dynamicQuestionLoading, setDynamicQuestionLoading] = useState(false); const [dynamicQuestionError, setDynamicQuestionError] = useState(''); const recorder = useRef<MediaRecorder | null>(null); const chunks = useRef<Blob[]>([]); const stream = useRef<MediaStream | null>(null); const fullRecorder = useRef<MediaRecorder | null>(null); const fullChunks = useRef<Blob[]>([]); const fullAudioRef = useRef<Blob | null>(null); const finishedRef = useRef(false); const speechRunId = useRef(0); const audioContext = useRef<AudioContext | null>(null); const [liveTranscript, setLiveTranscript] = useState(''); const [realtimeStatus, setRealtimeStatus] = useState(''); const realtimeSocket = useRef<WebSocket | null>(null); const realtimeAudioContext = useRef<AudioContext | null>(null); const realtimeSource = useRef<MediaStreamAudioSourceNode | null>(null); const realtimeProcessor = useRef<ScriptProcessorNode | null>(null); const realtimeMute = useRef<GainNode | null>(null); const realtimeRun = useRef(0); const liveTranscriptRef = useRef(''); const liveSegmentsRef = useRef<TranscriptSegment[]>([]); const realtimeFailedRef = useRef(false); const segmentStopResolver = useRef<((blob: Blob | null) => void) | null>(null);
  const playCue = useCallback((kind: 'countdown' | 'recording', value = 0) => {
    try {
      const AudioContextCtor = window.AudioContext || (window as typeof window & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext;
      if (!AudioContextCtor) return;
  ...
  async function startRealtimeTranscription(media: MediaStream) {
    ...
    liveTranscriptRef.current = ''; liveSegmentsRef.current = []; realtimeFailedRef.current = false; setLiveTranscript('');
    ...
    try {
      ...
      socket.onmessage = (event) => {
        ...
        try {
          ...
          if (message.type === 'error') { realtimeFailedRef.current = true; setRealtimeStatus(`实时转写不可用：${message.error}`); }
          if (message.type === 'closed' && !liveTranscriptRef.current && !realtimeFailedRef.current) setRealtimeStatus(`实时转写连接已结束：${message.reason || message.code || '未知原因'}`);
  ...
  async function generateDynamicQuestion(index = stepIndex, priorDrafts = drafts, sessionOverride = sessionId, stepsOverride = steps) {
    const target = stepsOverride[index];
    if (!target || target.kind !== 'dynamic' || !sessionOverride || dynamicQuestionLoading) return;
    setDynamicQuestionLoading(true); setDynamicQuestionError(''); setMessage('正在根据此前回答生成自由交流问题…');
    try {
      const data = await jsonFetch('/api/simulations/' + sessionOverride + '/dynamic-question', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ moduleId: target.templateModuleId || target.id, priorAnswers: priorDrafts.map((item) => ({ moduleTitle: item.moduleTitle, question: item.question, answer: item.answer, transcript: item.transcript })) }) });
      const question = String(data.question || '').trim();
      if (!question) throw new Error('未生成有效问题');
      setSteps((items) => items.map((item, position) => position === index ? { ...item, question } : item));
      setStepStartedAt(Date.now()); setPromptCycle((value) => value + 1); setMessage('');
    } catch (error) { const text = (error as Error).message; setDynamicQuestionError(text); setMessage(text); }
    finally { setDynamicQuestionLoading(false); }
  }
  async function start(startTemplateId = templateId) { try { const data = await jsonFetch('/api/simulations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ templateId: Number(startTemplateId) }) }); const firstDynamic = data.steps?.[0]?.kind === 'dynamic'; setSessionId(data.sessionId); setSteps(data.steps); setStepIndex(0); setStepStartedAt(firstDynamic ? 0 : Date.now()); setElapsed(0); setDrafts([]); setAnswer(''); setFollowup(null); setFollowupGenerating(false); setFollowupSourceAnswer(''); setSegmentBlob(null); setFullAudio(null); fullAudioRef.current = null; finishedRef.current = false; setReading(false); setDynamicQuestionError(''); setCountdown(firstDynamic ? null : 3); await startFullRecording(); if (firstDynamic) void generateDynamicQuestion(0, [], data.sessionId, data.steps); } catch (error) { setMessage((error as Error).message); } }
  async function startRecording() { try { const media = stream.current || await navigator.mediaDevices.getUserMedia({ audio: true }); stream.current = media; chunks.current = []; const value = new MediaRecorder(media); value.ondataavailable = (event) => { if (event.data.size) chunks.current.push(event.data); }; value.onstop = () => { const blob = new Blob(chunks.current, { type: value.mimeType || 'audio/webm' }); setSegmentBlob(blob); setRecording(false); segmentStopResolver.current?.(blob); segmentStopResolver.current = null; }; recorder.current = value; value.start(); setRecording(true); void startRealtimeTranscription(media); } catch { setMessage('无法使用麦克风，请先允许浏览器录音权限。'); } }
  ...
  async function stopRecording() { stopRealtimeTranscription(); const value = recorder.current; if (!value || value.state !== 'recording') return segmentBlob; return new Promise<Blob | null>((resolve) => { segmentStopResolver.current = resolve; value.stop(); }); }
  ...
  async function generateFollowup(sourceAnswer = answer, primaryAlreadySaved = false) { if (!current || !sourceAnswer.trim() || followupGenerating) return false; if (!primaryAlreadySaved) { saveCurrent(current.title, current.question || current.prompt || ''); } setFollowupGenerating(true); try { const data = await jsonFetch('/api/simulations/' + sessionId + '/followup', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question: current.question || current.prompt, answer: sourceAnswer, moduleTitle: current.title }) }); setFollowup(data.followup); return true; } catch (error) { setMessage((error as Error).message); return false; } finally { setFollowupGenerating(false); } }
  function saveCurrent(title = current?.title || '', question = followup || current?.question || current?.prompt || '', followupQuestion?: string, audioOverride: Blob | null = segmentBlob) { if (!current) return null; const transcript = liveTranscriptRef.current || answer; const draft = { moduleIndex: stepIndex, moduleTitle: followup ? current.title + ' · 老师追问' : title, questionId: current.questionId, question, answer, transcript, transcriptSegments: liveSegmentsRef.current.length ? [...liveSegmentsRef.current] : undefined, elapsedSeconds: Math.max(0, Math.floor((Date.now() - stepStartedAt) / 1000)), audio: audioOverride || undefined, followupQuestion }; setDrafts((items) => [...items, draft]); setAnswer(''); setSegmentBlob(null); setFollowup(null); stopRealtimeTranscription(true); liveTranscriptRef.current = ''; liveSegmentsRef.current = []; setLiveTranscript(''); setRealtimeStatus(''); return draft; }
  async function beginFollowup(audioOverride?: Blob | null) { const audio = audioOverride === undefined ? (recording ? await stopRecording() : segmentBlob) : audioOverride; const sourceAnswer = liveTranscriptRef.current || answer || followupSourceAnswer; if (!sourceAnswer.trim()) return; if (!followupSourceAnswer) { setFollowupSourceAnswer(sourceAnswer); saveCurrent(current?.title || '', current?.question || current?.prompt || '', undefined, audio); } if (await generateFollowup(sourceAnswer, true)) { setStepStartedAt(Date.now()); setPromptCycle((value) => value + 1); } }
  async function next() { if (current?.allowFollowup && (!followup || followupGenerating)) return; if (followup && !(liveTranscript.trim() || answer.trim() || segmentBlob)) return; const audio = recording ? await stopRecording() : segmentBlob; const saved = saveCurrent(undefined, undefined, followup || undefined, audio); const nextDrafts = saved ? [...drafts, saved] : drafts; setFollowupSourceAnswer(''); if (stepIndex + 1 < steps.length) { const nextIndex = stepIndex + 1; const nextStep = steps[nextIndex]; setStepIndex(nextIndex); setDynamicQuestionError(''); if (nextStep?.kind === 'dynamic' && !nextStep.question) { setStepStartedAt(0); setCountdown(null); void generateDynamicQuestion(nextIndex, nextDrafts); } else setStepStartedAt(Date.now()); } else await finish(nextDrafts); }
  async function exitSimulation() {
    speechRunId.current += 1; window.speechSynthesis?.cancel();
  ...
  const hasCurrentResponse = Boolean(liveTranscript.trim() || answer.trim() || segmentBlob);
  const waitingForFollowup = Boolean(current.allowFollowup && !followup);
  const followupResponseMissing = Boolean(followup && !hasCurrentResponse);
  const nextDisabled = Boolean(countdown !== null || reading || followupGenerating || waitingForFollowup || followupResponseMissing);
  const nextHint = followupGenerating ? '老师正在生成追问，请稍候。' : waitingForFollowup ? '本环节已设置老师追问，请先生成并完成追问。' : followupResponseMissing ? '请先完成老师追问的作答，再进入下一环节。' : followup ? '追问已完成，可以保存并进入下一环节。' : '确认本段作答后，保存并进入下一环节。';
  const prompt = followup || current.question || current.prompt || '请开始作答。';
  return <main className="simulation-page running"><div className="simulation-running-head"><button className="back" onClick={() => void exitSimulation()}>退出模拟</button><div><span>全程倒计时</span><strong>{format(totalSeconds - elapsed)}</strong></div><div><span>本环节建议时长</span><strong className={stepElapsed > (current.timeSeconds || 0) ? 'overtime' : ''}>{format(current.timeSeconds || 0)}</strong></div></div><ol className="simulation-puzzle">{steps.map((item, index) => <li className={index === stepIndex ? 'active' : index < stepIndex ? 'done' : ''} key={item.id}><b>{String(index + 1).padStart(2, '0')}</b><span>{item.title}</span></li>)}</ol>{countdown !== null && <div className="simulation-countdown-overlay" role="status" aria-live="assertive"><div className="simulation-countdown-number" key={countdown}>{countdown}</div><strong>准备开始</strong><small>{readQuestion ? (autoRecord ? '题目朗读结束后将自动开始录音' : '题目朗读结束后开始作答') : (autoRecord ? '倒计时结束后将自动开始录音' : '倒计时结束后开始作答')}</small></div>}<section className="simulation-question"><span className="section-kicker">{followup ? 'TEACHER FOLLOW-UP' : current.title}</span><h1>{dynamicPending ? '正在准备自由交流问题…' : prompt}</h1>{current.subcategory && <small>{current.subcategory}</small>}<p>{followup ? '这是基于刚才回答生成的老师追问，请继续作答。' : dynamicPending ? '系统会结合此前的自我介绍、项目介绍和作答内容生成问题；不会在此阶段开启录音。' : current.kind === 'dynamic' ? '这道自由交流题由此前回答动态生成。' : '本环节超过建议时长只会提醒；全程时间到后应结束模拟。'}</p>{dynamicPending && <div className="simulation-generating"><span>{dynamicQuestionLoading ? '正在调用模型生成问题，请稍候…' : dynamicQuestionError || '正在准备问题…'}</span>{!dynamicQuestionLoading && <button type="button" onClick={() => void generateDynamicQuestion()}>重新生成问题</button>}</div>}</section>{!dynamicPending && <><section className="simulation-answer"><textarea disabled={countdown !== null || reading} value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder="可选：记录回答提纲；录音和实时文字稿将保存到本场模拟。" />{realtimeStatus && <div className="realtime-status">{realtimeStatus}</div>}{liveTranscript && <div className="live-transcript"><small>实时识别稿</small><p>{liveTranscript}</p></div>}<div className="recorder"><span><b>{recording ? '正在分段录音' : segmentBlob ? '本段录音已完成' : '准备录制本段回答'}</b><small>当前模块：{current.title}</small></span><button disabled={countdown !== null || reading} onClick={() => recording ? void stopRecording() : void startRecording()}>{recording ? '结束本段录音' : '开始本段录音'}</button></div>{current.allowFollowup && !followup && <button className="secondary-action" disabled={!(hasCurrentResponse || followupSourceAnswer.trim()) || followupGenerating || countdown !== null || reading} onClick={() => void beginFollowup()}>{followupGenerating ? '正在生成老师追问…' : followupSourceAnswer ? '重新生成老师追问' : '生成老师追问'}</button>}</section><div className="simulation-actions"><div><strong>{followup ? '老师追问' : '本段作答'}</strong><small>{nextHint}</small></div><button disabled={nextDisabled} onClick={() => void next()}>{followup ? '完成追问并进入下一环节 →' : stepIndex + 1 === steps.length ? '完成模拟并保存 →' : '保存本段并进入下一环节 →'}</button></div></>}</main>;
}


function SimulationLobby({ templates, templateId, onSelect, onStart, onBack, format }: { templates: SimulationTemplate[]; templateId: string; onSelect: (value: string) => void; onStart: (id: number) => void; onBack: () => void; format: (seconds: number) => string }) {
  return <main className="simulation-page"><button className="back" onClick={onBack}>← 返回练习方式</button><header><span className="section-kicker">REAL INTERVIEW SIMULATION</span><h1>真实场景模拟</h1><p>选择一个学校的面试流程，按模块完成整场练习；每次模拟都会独立保存。</p></header><div className="simulation-template-list">{templates.map((item) => <article key={item.id}><div><strong>{item.name}</strong><span>{item.description || '暂无流程说明'}</span><small>总时长 {format(item.totalSeconds)} · {Array.isArray(item.modules) ? item.modules.length : 0} 个模块</small></div><button className="simulation-enter" onClick={() => { onSelect(String(item.id)); onStart(item.id); }}>进入模拟</button></article>)}</div>{!templates.length && <div className="empty"><h3>暂无可用流程</h3><p>请管理员在后台添加学校面试流程。</p></div>}</main>;
}
...
function LegacySimulationHistory({ onBack }: { onBack: () => void }) {
  ...
  const [message, setMessage] = useState(''); const [chatOpen, setChatOpen] = useState(false); const [chatInput, setChatInput] = useState(''); const [chatLoading, setChatLoading] = useState(false); const [generating, setGenerating] = useState(false);
  useEffect(() => { void jsonFetch('/api/simulation-records').then((data) => setRecords(data.records || [])).catch((error) => setMessage((error as Error).message)); }, []);
  const filteredRecords = templateFilter ? records.filter((item) => item.templateName === templateFilter) : records;
  const loadReview = useCallback(async (sessionId: number) => { const data = await jsonFetch('/api/ai/simulation-evaluations?sessionId=' + sessionId); setEvaluations(data.evaluations || []); setChatMessages((data.messages || []).map((item: { id: number; role: 'user' | 'assistant'; content: string }) => ({ id: Number(item.id), role: item.role, content: item.content }))); }, []);
  ...
  async function openRecord(item: SimulationRecord) { setSelected(item); setDetail(null); setEvaluations([]); setChatMessages([]); try { const [record] = await Promise.all([jsonFetch('/api/simulation-records/' + item.id), loadReview(item.id)]); setDetail(record); } catch (error) { setMessage((error as Error).message); } }
  async function generateReview() { if (!selected || generating) return; setGenerating(true); setMessage(''); try { const data = await jsonFetch('/api/ai/simulation-evaluations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: selected.id }) }); setMessage(data.reused ? '本场模拟没有新的回答，已保留现有复盘。' : '模拟复盘已提交，正在生成，请稍候。'); await loadReview(selected.id); } catch (error) { setMessage((error as Error).message); } finally { setGenerating(false); } }
  async function sendChat(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const content = chatInput.trim(); if (!content || !selected || chatLoading) return; const clientId = -Date.now(); setChatLoading(true); setMessage(''); setChatInput(''); setChatMessages((current) => [...current, { id: clientId - 1, role: 'user', content }, { id: clientId, role: 'assistant', content: '' }]); try { const response = await fetch('/api/ai/simulation-chat', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'text/event-stream' }, body: JSON.stringify({ sessionId: selected.id, message: content }) }); if (!response.ok) { const payload = await response.json().catch(() => ({})); throw new Error(payload.error || '发送失败'); } const reader = response.body?.getReader(); if (!reader) throw new Error('浏览器不支持流式响应'); const decoder = new TextDecoder(); let buffer = ''; const applyEvent = (raw: string) => { const line = raw.split(/\r?\n/).find((item) => item.startsWith('data:')); if (!line) return; const payload = JSON.parse(line.slice(5).trim()) as { type?: string; content?: string; error?: string }; if (payload.type === 'error') throw new Error(payload.error || '对话生成失败'); if (payload.type === 'delta' && payload.content) setChatMessages((current) => current.map((item) => item.id === clientId ? { ...item, content: item.content + payload.content } : item)); }; while (true) { const { value, done } = await reader.read(); if (done) break; buffer += decoder.decode(value, { stream: true }); const events = buffer.split(/\r?\n\r?\n/); buffer = events.pop() || ''; events.forEach(applyEvent); } if (buffer.trim()) applyEvent(buffer); await loadReview(selected.id); } catch (error) { setMessage((error as Error).message); setChatMessages((current) => current.filter((item) => item.id !== clientId)); } finally { setChatLoading(false); } }
  if (selected && detail) { const completed = evaluations.some((item) => item.status === 'completed'); return <main className="simulation-page"><button className="back" onClick={() => { setSelected(null); setDetail(null); }}>← 返回真实模拟记录</button><header className="simulation-detail-head"><div><span className="section-kicker">SIMULATION REVIEW</span><h1>{detail.session.templateName}</h1><p>{detail.session.username ? `学员：${detail.session.username} · ` : ''}{formatRecordDate(detail.session.startedAt)} · 用时 {Math.floor(detail.session.elapsedSeconds / 60)} 分钟</p>{detail.session.hasFullAudio ? <AudioWithDuration src={'/api/simulation-records/' + detail.session.id + '/audio'} className="simulation-full-audio" /> : null}</div><div className="simulation-review-actions"><button className="review-generate" disabled={generating} onClick={() => void generateReview()}>{generating ? '正在提交…' : completed ? '更新模拟复盘' : '复盘模拟流程'}</button><button className="simulation-chat-trigger" onClick={() => setChatOpen(true)}>和小鱼讨论</button></div></header>{message && <div className="management-message">{message}</div>}<section className="simulation-review-evaluations review-section"><div className="review-section-title"><div><span className="section-kicker">SIMULATION EVALUATION</span><h2>整场复盘</h2></div><small>{evaluations.length ? `${evaluations.length} 次评估` : '尚未生成'}</small></div>{evaluations.length ? evaluations.map((item, index) => <details className="evaluation-card" key={item.id} open={index === 0}><summary><span><strong>{index === 0 ? '最新复盘' : '历史复盘 ' + (evaluations.length - index)}</strong><small>{formatRecordDate(item.createdAt)}</small></span><em>{item.status === 'processing' ? '生成中' : item.status === 'completed' ? '已完成' : '失败'}</em></summary>{item.status === 'processing' && <p className="evaluation-pending">小鱼正在分析整场模拟，请稍候，页面会自动刷新。</p>}{item.status === 'failed' && <p className="evaluation-error">{item.error || '本次复盘失败'}</p>}{item.status === 'completed' && <MarkdownContent value={item.result || ''} className="evaluation-result" />}</details>) : <div className="review-empty">点击“复盘模拟流程”后，会使用管理后台当前的评估提示词分析整场表现。</div>}</section><section className="simulation-answer-list">{detail.answers.map((item) => <article key={item.id}><div className="simulation-answer-head"><span>模块 {item.moduleIndex + 1} · {item.moduleTitle}</span><small>{formatRecordDate(item.createdAt)} · {item.elapsedSeconds}s</small></div><h2>{item.question}</h2><p>{item.transcript || item.answer || '暂无文字回答'}</p>{item.hasAudio ? <AudioWithDuration src={'/api/simulation-records/' + detail.session.id + '/audio/' + item.id} /> : null}</article>)}</section>{chatOpen && <div className="chat-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !chatLoading) setChatOpen(false); }}><section className="chat-modal" role="dialog" aria-modal="true" aria-labelledby="simulation-chat-modal-title"><div className="chat-modal-head"><div><span className="section-kicker">SIMULATION DISCUSSION</span><h2 id="simulation-chat-modal-title">和小鱼讨论</h2><p>这里是正常讨论，不会自动重复整场评估；小鱼已了解本场题目、回答和转写。</p></div><button className="chat-modal-close" aria-label="关闭对话" disabled={chatLoading} onClick={() => setChatOpen(false)}>×</button></div><div className="chat-messages">{chatMessages.length ? chatMessages.map((item) => <div className={'chat-message ' + item.role} key={item.id}><MarkdownContent value={item.content || (chatLoading ? '正在思考…' : '')} /></div>) : <div className="chat-empty">可以追问某个回答如何改写、某个停顿如何处理，或让小鱼继续扮演老师。</div>}</div><form className="chat-form" onSubmit={sendChat}><textarea value={chatInput} onChange={(event) => setChatInput(event.target.value)} placeholder="例如：把我的英文回答改成更自然的 60 秒版本。" disabled={chatLoading} /><button disabled={chatLoading || !chatInput.trim()}>{chatLoading ? '生成中…' : '发送'}</button></form></section></div>}</main>; }
  return <main className="simulation-page"><button className="back" onClick={onBack}>← 返回练习方式</button><header><span className="section-kicker">SIMULATION HISTORY</span><h1>真实模拟记录</h1><p>按学校流程查看每一次完整面试模拟；管理员可以查看所有学员记录。</p></header>{records.length > 0 && <div className="simulation-history-filter"><label>按学校流程筛选<select value={templateFilter} onChange={(event) => setTemplateFilter(event.target.value)}><option value="">全部流程</option>{Array.from(new Set(records.map((item) => item.templateName))).map((name) => <option key={name} value={name}>{name}</option>)}</select></label></div>}{message && <div className="management-message">{message}</div>}<div className="simulation-history-list">{filteredRecords.map((item) => <article key={item.id}><div><span className="tag-chip">{item.templateName}</span>{item.username && <span className="tag-chip">{item.username}</span>}<h2>{formatRecordDate(item.startedAt)}</h2><p>{item.answerCount || 0} 个回答 · 用时 {Math.floor(item.elapsedSeconds / 60)} 分钟</p></div><button onClick={() => void openRecord(item)}>查看复盘</button></article>)}</div>{!records.length && <div className="empty"><h3>还没有真实模拟记录</h3><p>完成一次学校流程模拟后，记录会显示在这里。</p></div>}</main>;
}
...
type ApiUsageItem = { inputTokens: number; outputTokens: number; audioSeconds: number; requestCount: number };
type ApiUsageUser = { id: number; username: string; displayName: string; aiEnabled: boolean; asrEnabled: boolean; realtimeAsrEnabled: boolean; aiTokenLimit: number; asrRequestLimit: number; realtimeSecondsLimit: number; usage: Record<string, ApiUsageItem> };
type ApiUsageDay = { label: string; aiTokens: number; asrRequests: number; realtimeSeconds: number };

function UsageLineChart({ days }: { days: ApiUsageDay[] }) {
  const max = Math.max(1, ...days.map((item) => item.aiTokens)); const width = 520; const height = 210; const pad = 26;
  const points = days.map((item, index) => `${pad + index * ((width - pad * 2) / Math.max(1, days.length - 1))},${height - pad - item.aiTokens / max * (height - pad * 2)}`).join(' ');
  return <figure className="usage-chart-card"><figcaption><span>大模型 Tokens</span><strong>{days.reduce((total, item) => total + item.aiTokens, 0).toLocaleString()}</strong><small>最近 7 天 · 含对话、评估与追问</small></figcaption><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="最近七天大模型 token 用量折线图"><line x1={pad} x2={width - pad} y1={pad} y2={pad} /><line x1={pad} x2={width - pad} y1={height / 2} y2={height / 2} /><line x1={pad} x2={width - pad} y1={height - pad} y2={height - pad} /><polyline className="usage-line-fill" points={`${pad},${height - pad} ${points} ${width - pad},${height - pad}`} /><polyline className="usage-line" points={points} />{days.map((item, index) => { const x = pad + index * ((width - pad * 2) / Math.max(1, days.length - 1)); const y = height - pad - item.aiTokens / max * (height - pad * 2); return <g key={item.label}><circle cx={x} cy={y} r="3.5"><title>{item.label}：{item.aiTokens.toLocaleString()} token</title></circle><text x={x} y={height - 7} textAnchor="middle">{item.label}</text></g>; })}</svg></figure>;
}

function UsageBarChart({ days }: { days: ApiUsageDay[] }) {
  const max = Math.max(1, ...days.map((item) => Math.max(item.asrRequests, item.realtimeSeconds))); const width = 520; const height = 210; const pad = 26; const group = (width - pad * 2) / Math.max(1, days.length);
...
function LegacyUsageManagement() {
  ...
  useEffect(() => { void load(); }, [load]);
  const update = (id: number, patch: Partial<ApiUsageUser>) => setUsers((items) => items.map((item) => item.id === id ? { ...item, ...patch } : item));
  const amount = (user: ApiUsageUser, feature: 'ai' | 'asr' | 'realtime_asr') => feature === 'ai' ? Number(user.usage.ai?.inputTokens || 0) + Number(user.usage.ai?.outputTokens || 0) : feature === 'asr' ? Number(user.usage.asr?.requestCount || 0) : Number(user.usage.realtime_asr?.audioSeconds || 0);
  const ratio = (used: number, limit: number) => limit > 0 ? Math.min(100, Math.round(used / limit * 100)) : 0;
  const formatSeconds = (value: number) => value >= 60 ? `${Math.floor(value / 60)} 分 ${value % 60} 秒` : `${value} 秒`;
  async function save(user: ApiUsageUser) { setSavingId(user.id); setMessage(''); try { await jsonFetch('/api/usage', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, aiEnabled: user.aiEnabled, asrEnabled: user.asrEnabled, realtimeAsrEnabled: user.realtimeAsrEnabled, aiTokenLimit: user.aiTokenLimit, asrRequestLimit: user.asrRequestLimit, realtimeSecondsLimit: user.realtimeSecondsLimit }) }); setMessage(`已保存 ${user.displayName} 的 API 权限与额度`); await load(); } catch (error) { setMessage((error as Error).message); } finally { setSavingId(0); } }
  return <section className="usage-management"><header className="usage-head"><div><span className="section-kicker">API USAGE & GUARDRAILS</span><h2>用量与额度</h2><p>图表展示最近 7 天的实际消耗；用户额度按自然月控制，填 <b>0</b> 表示不限额。</p></div><button className="secondary-action" onClick={() => void load()}>刷新数据</button></header>{message && <div className="management-message" role="status">{message}</div>}<div className="usage-charts"><UsageLineChart days={daily} /><UsageBarChart days={daily} /></div><div className="usage-user-list">{users.map((user) => { const aiUsed = amount(user, 'ai'); const asrUsed = amount(user, 'asr'); const realtimeUsed = amount(user, 'realtime_asr'); return <article className="usage-user-card" key={user.id}><div className="usage-user-heading"><div className="usage-avatar">{user.displayName.slice(0, 1)}</div><div><h3>{user.displayName}</h3><span>@{user.username}</span></div><button className="modal-submit usage-save" disabled={savingId === user.id} onClick={() => void save(user)}>{savingId === user.id ? '保存中…' : '保存此用户'}</button></div><div className="usage-meters"><div className="usage-meter"><div><strong>大模型</strong><span>{aiUsed.toLocaleString()} / {user.aiTokenLimit ? user.aiTokenLimit.toLocaleString() : '不限额'} token</span></div><div className="usage-bar"><i style={{ width: `${ratio(aiUsed, user.aiTokenLimit)}%` }} /></div></div><div className="usage-meter"><div><strong>录音转写</strong><span>{asrUsed} / {user.asrRequestLimit || '不限额'} 次</span></div><div className="usage-bar amber"><i style={{ width: `${ratio(asrUsed, user.asrRequestLimit)}%` }} /></div></div><div className="usage-meter"><div><strong>实时转写</strong><span>{formatSeconds(realtimeUsed)} / {user.realtimeSecondsLimit ? formatSeconds(user.realtimeSecondsLimit) : '不限额'}</span></div><div className="usage-bar violet"><i style={{ width: `${ratio(realtimeUsed, user.realtimeSecondsLimit)}%` }} /></div></div></div><div className="usage-controls"><label><input type="checkbox" checked={user.aiEnabled} onChange={(event) => update(user.id, { aiEnabled: event.target.checked })} /> 启用 AI 对话、评估与追问</label><label>每月 token 额度<input type="number" min="0" value={user.aiTokenLimit} onChange={(event) => update(user.id, { aiTokenLimit: Math.max(0, Number(event.target.value) || 0) })} /></label><label><input type="checkbox" checked={user.asrEnabled} onChange={(event) => update(user.id, { asrEnabled: event.target.checked })} /> 启用录音转文字</label><label>每月转写任务额度<input type="number" min="0" value={user.asrRequestLimit} onChange={(event) => update(user.id, { asrRequestLimit: Math.max(0, Number(event.target.value) || 0) })} /></label><label><input type="checkbox" checked={user.realtimeAsrEnabled} onChange={(event) => update(user.id, { realtimeAsrEnabled: event.target.checked })} /> 启用实时转写</label><label>每月实时转写秒数<input type="number" min="0" value={user.realtimeSecondsLimit} onChange={(event) => update(user.id, { realtimeSecondsLimit: Math.max(0, Number(event.target.value) || 0) })} /></label></div></article>; })}</div>{!users.length && <div className="review-empty">暂无可统计用户。</div>}</section>;
}
function Management() {
  const [tab, setTab] = useState<'users' | 'questions' | 'ai' | 'simulation' | 'usage'>('users');
...
function LegacySimulationConfig() {
  ...
  async function saveTemplate() { if (!selected) return; try { await jsonFetch('/api/simulations/config', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ template: selected }) }); setMessage('模拟流程已保存'); setSelected(null); await load(); } catch (error) { setMessage((error as Error).message); } }
