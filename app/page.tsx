...
function Simulation({ onBack }: { onBack: () => void }) {
  ...
  async function generateFollowup(sourceAnswer = answer, primaryAlreadySaved = false) { if (!current || !sourceAnswer.trim()) return false; if (!primaryAlreadySaved) { saveCurrent(current.title, current.question || current.prompt || ''); } try { const data = await jsonFetch('/api/simulations/' + sessionId + '/followup', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question: current.question || current.prompt, answer: sourceAnswer, moduleTitle: current.title }) }); setFollowup(data.followup); return true; } catch (error) { setMessage((error as Error).message); return false; } }
  ...
  async function beginFollowup(audioOverride?: Blob | null) { const audio = audioOverride === undefined ? (recording ? await stopRecording() : segmentBlob) : audioOverride; const sourceAnswer = liveTranscriptRef.current || answer; saveCurrent(current?.title || '', current?.question || current?.prompt || '', undefined, audio); if (await generateFollowup(sourceAnswer, true)) { setStepStartedAt(Date.now()); setPromptCycle((value) => value + 1); } }
  async function next() { const audio = recording ? await stopRecording() : segmentBlob; if (current?.allowFollowup && (liveTranscript.trim() || answer.trim()) && !followup) { await beginFollowup(audio); return; } const saved = saveCurrent(undefined, undefined, followup || undefined, audio); const nextDrafts = saved ? [...drafts, saved] : drafts; if (stepIndex + 1 < steps.length) { const nextIndex = stepIndex + 1; const nextStep = steps[nextIndex]; setStepIndex(nextIndex); setDynamicQuestionError(''); if (nextStep?.kind === 'dynamic' && !nextStep.question) { setStepStartedAt(0); setCountdown(null); void generateDynamicQuestion(nextIndex, nextDrafts); } else setStepStartedAt(Date.now()); } else await finish(nextDrafts); }
