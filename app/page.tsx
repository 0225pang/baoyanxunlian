...
function SimulationHistory({ onBack }: { onBack: () => void }) {
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ id: number; role: 'user' | 'assistant'; content: string }>>([]);
  ...
  async function openRecord(item: SimulationRecord) {
    setSelected(item); setDetail(null); setChatMessages([]);
    try { setDetail(await jsonFetch('/api/simulation-records/' + item.id)); } catch (error) { setMessage((error as Error).message); }
  }
  async function sendChat(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const content = chatInput.trim();
    if (!content || !selected || chatLoading) return;
    const clientId = -Date.now(); setChatLoading(true); setMessage(''); setChatInput('');
    setChatMessages((current) => [...current, { id: clientId - 1, role: 'user', content }, { id: clientId, role: 'assistant', content: '' }]);
    try {
      const response = await fetch('/api/ai/simulation-chat', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'text/event-stream' }, body: JSON.stringify({ sessionId: selected.id, message: content }) });
      if (!response.ok) { const payload = await response.json().catch(() => ({})); throw new Error(payload.error || '发送失败'); }
      ...
      const applyEvent = (raw: string) => {
        ...
        if (payload.type === 'delta' && payload.content) setChatMessages((current) => current.map((item) => item.id === clientId ? { ...item, content: item.content + payload.content } : item));
      };
      while (true) { const { value, done } = await reader.read(); if (done) break; buffer += decoder.decode(value, { stream: true }); const events = buffer.split(/\r?\n\r?\n/); buffer = events.pop() || ''; events.forEach(applyEvent); }
      if (buffer.trim()) applyEvent(buffer);
    } catch (error) { setMessage((error as Error).message); setChatMessages((current) => current.filter((item) => item.id !== clientId)); }
    finally { setChatLoading(false); }
  }
  if (selected && detail) return <main className="simulation-page"><button className="back" onClick={() => { setSelected(null); setDetail(null); }}>← 返回真实模拟记录</button><header className="simulation-detail-head"><div><span className="section-kicker">SIMULATION REVIEW</span><h1>{detail.session.templateName}</h1><p>{detail.session.username ? `学员：${detail.session.username} · ` : ''}{formatRecordDate(detail.session.startedAt)} · 用时 {Math.floor(detail.session.elapsedSeconds / 60)} 分钟</p>{detail.session.hasFullAudio ? <AudioWithDuration src={'/api/simulation-records/' + detail.session.id + '/audio'} className="simulation-full-audio" /> : null}</div><button className="simulation-chat-trigger" onClick={() => setChatOpen(true)}>和小鱼讨论</button></header><section className="simulation-answer-list">{detail.answers.map((item) => <article key={item.id}><div className="simulation-answer-head"><span>模块 {item.moduleIndex + 1} · {item.moduleTitle}</span><small>{formatRecordDate(item.createdAt)} · {item.elapsedSeconds}s</small></div><h2>{item.question}</h2><p>{item.transcript || item.answer || '暂无文字回答'}</p>{item.hasAudio ? <AudioWithDuration src={'/api/simulation-records/' + detail.session.id + '/audio/' + item.id} /> : null}</article>)}</section>{chatOpen && <div className="chat-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !chatLoading) setChatOpen(false); }}><section className="chat-modal" role="dialog" aria-modal="true" aria-labelledby="simulation-chat-modal-title"><div className="chat-modal-head"><div><span className="section-kicker">SIMULATION DISCUSSION</span><h2 id="simulation-chat-modal-title">和小鱼讨论</h2><p>围绕这场完整模拟打磨表达、分析节奏或继续模拟追问。</p></div><button className="chat-modal-close" aria-label="关闭对话" disabled={chatLoading} onClick={() => setChatOpen(false)}>×</button></div><div className="chat-messages">{chatMessages.length ? chatMessages.map((item) => <div className={'chat-message ' + item.role} key={item.id}><MarkdownContent value={item.content || (chatLoading ? '正在思考…' : '')} /></div>) : <div className="chat-empty">还没有对话。可以让小鱼评价整场回答、帮你优化某个环节，或继续扮演面试老师追问。</div>}</div><form className="chat-form" onSubmit={sendChat}><textarea value={chatInput} onChange={(event) => setChatInput(event.target.value)} placeholder="例如：请重点评价我的自我介绍，并给出 90 秒优化版本。" disabled={chatLoading} /><button disabled={chatLoading || !chatInput.trim()}>{chatLoading ? '生成中…' : '发送'}</button></form></section></div>}</main>;
  return <main className="simulation-page"><button className="back" onClick={onBack}>← 返回练习方式</button><header><span className="section-kicker">SIMULATION HISTORY</span><h1>真实模拟记录</h1><p>按学校流程查看每一次完整面试模拟；管理员可以查看所有学员记录。</p></header>{records.length > 0 && <div className="simulation-history-filter"><label>按学校流程筛选<select value={templateFilter} onChange={(event) => setTemplateFilter(event.target.value)}><option value="">全部流程</option>{Array.from(new Set(records.map((item) => item.templateName))).map((name) => <option key={name} value={name}>{name}</option>)}</select></label></div>}{message && <div className="management-message">{message}</div>}<div className="simulation-history-list">{filteredRecords.map((item) => <article key={item.id}><div><span className="tag-chip">{item.templateName}</span>{item.username && <span className="tag-chip">{item.username}</span>}<h2>{formatRecordDate(item.startedAt)}</h2><p>{item.status === 'completed' ? '已完成' : '进行中'} · {item.answerCount || 0} 个回答 · 用时 {Math.floor(item.elapsedSeconds / 60)} 分钟</p></div><button onClick={() => void openRecord(item)}>查看复盘</button></article>)}</div>{!records.length && <div className="empty"><h3>还没有真实模拟记录</h3><p>完成一次学校流程模拟后，记录会显示在这里。</p></div>}</main>;
}
...
function UsageManagement() {
  ...
  const [totals, setTotals] = useState({ aiTokens: 0, asrRequests: 0, realtimeSeconds: 0 });
  const [month, setMonth] = useState(''); const [message, setMessage] = useState(''); const [savingId, setSavingId] = useState(0);
  const load = useCallback(async () => { try { const data = await jsonFetch('/api/usage'); setUsers(data.users || []); setTotals(data.totals || { aiTokens: 0, asrRequests: 0, realtimeSeconds: 0 }); setMonth(data.month || ''); } catch (error) { setMessage((error as Error).message); } }, []);
  useEffect(() => { void load(); }, [load]);
  ...
  return <section className="usage-management"><header className="usage-head"><div><span className="section-kicker">API USAGE & GUARDRAILS</span><h2>用量与额度</h2><p>按自然月统计。大模型以 token 计，普通转写以任务次数计，实时转写以实际连接时长计；额度填 <b>0</b> 表示不限额。</p></div><button className="secondary-action" onClick={() => void load()}>刷新数据</button></header>{message && <div className="management-message" role="status">{message}</div>}<div className="usage-overview" aria-label="本月全站 API 用量概览"><article><span>大模型 token</span><strong>{totals.aiTokens.toLocaleString()}</strong><small>{month || '本月'} · 对话、评估与追问</small></article><article><span>录音转写任务</span><strong>{totals.asrRequests.toLocaleString()}</strong><small>Paraformer 等异步转写</small></article><article><span>实时转写时长</span><strong>{formatSeconds(totals.realtimeSeconds)}</strong><small>真实模拟的流式语音识别</small></article></div><div className="usage-user-list">{users.map((user) => { const aiUsed = amount(user, 'ai'); const asrUsed = amount(user, 'asr'); const realtimeUsed = amount(user, 'realtime_asr'); return <article className="usage-user-card" key={user.id}><div className="usage-user-heading"><div className="usage-avatar">{user.displayName.slice(0, 1)}</div><div><h3>{user.displayName}</h3><span>@{user.username}</span></div><button className="modal-submit usage-save" disabled={savingId === user.id} onClick={() => void save(user)}>{savingId === user.id ? '保存中…' : '保存此用户'}</button></div><div className="usage-meters"><div className="usage-meter"><div><strong>大模型</strong><span>{aiUsed.toLocaleString()} / {user.aiTokenLimit ? user.aiTokenLimit.toLocaleString() : '不限额'} token</span></div><div className="usage-bar" aria-label={`大模型用量 ${aiUsed} token`}><i style={{ width: `${ratio(aiUsed, user.aiTokenLimit)}%` }} /></div></div><div className="usage-meter"><div><strong>录音转写</strong><span>{asrUsed} / {user.asrRequestLimit || '不限额'} 次</span></div><div className="usage-bar amber" aria-label={`录音转写用量 ${asrUsed} 次`}><i style={{ width: `${ratio(asrUsed, user.asrRequestLimit)}%` }} /></div></div><div className="usage-meter"><div><strong>实时转写</strong><span>{formatSeconds(realtimeUsed)} / {user.realtimeSecondsLimit ? formatSeconds(user.realtimeSecondsLimit) : '不限额'}</span></div><div className="usage-bar violet" aria-label={`实时转写用量 ${formatSeconds(realtimeUsed)}`}><i style={{ width: `${ratio(realtimeUsed, user.realtimeSecondsLimit)}%` }} /></div></div></div><div className="usage-controls"><label><input type="checkbox" checked={user.aiEnabled} onChange={(event) => update(user.id, { aiEnabled: event.target.checked })} /> 启用 AI 对话、评估与追问</label><label>每月 token 额度<input type="number" min="0" value={user.aiTokenLimit} onChange={(event) => update(user.id, { aiTokenLimit: Math.max(0, Number(event.target.value) || 0) })} /></label><label><input type="checkbox" checked={user.asrEnabled} onChange={(event) => update(user.id, { asrEnabled: event.target.checked })} /> 启用录音转文字</label><label>每月转写任务额度<input type="number" min="0" value={user.asrRequestLimit} onChange={(event) => update(user.id, { asrRequestLimit: Math.max(0, Number(event.target.value) || 0) })} /></label><label><input type="checkbox" checked={user.realtimeAsrEnabled} onChange={(event) => update(user.id, { realtimeAsrEnabled: event.target.checked })} /> 启用实时转写</label><label>每月实时转写秒数<input type="number" min="0" value={user.realtimeSecondsLimit} onChange={(event) => update(user.id, { realtimeSecondsLimit: Math.max(0, Number(event.target.value) || 0) })} /></label></div></article>; })}</div>{!users.length && <div className="review-empty">暂无可统计用户。</div>}</section>;
}
function Management() {
