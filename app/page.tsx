...
function SimulationHistory({ onBack }: { onBack: () => void }) {
  ...
  async function sendChat(event: FormEvent) {
    event.preventDefault(); const text = chatInput.trim();
    if (!text || !selected || chatLoading) return;
    setChatInput(''); setChatMessages((items) => [...items, { role: 'user', content: text }, { role: 'assistant', content: '' }]); setChatLoading(true);
    try {
      const response = await fetch('/api/ai/simulation-chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sessionId: selected.id, message: text }) });
      if (!response.ok) throw new Error((await response.json()).error || 'AI 请求失败');
      if (!response.body) throw new Error('AI 未返回流');
      ...
      const consume = (line: string) => {
        ...
        if (payload.type === 'error') throw new Error(payload.error || 'AI 请求失败');
        if (payload.type === 'delta') { reply += payload.content || ''; setChatMessages((items) => items.map((item, index) => index === items.length - 1 ? { ...item, content: reply } : item)); }
      };
    ...
    } catch (error) { setMessage((error as Error).message); setChatMessages((items) => items.slice(0, -1)); }
    finally { setChatLoading(false); }
  }
  if (selected && detail) return <main className="simulation-page"><button className="back" onClick={() => { setSelected(null); setDetail(null); }}>← 返回真实模拟记录</button><header className="simulation-detail-head"><div><span className="section-kicker">SIMULATION REVIEW</span><h1>{detail.session.templateName}</h1><p>{detail.session.username ? `学员：${detail.session.username} · ` : ''}{formatRecordDate(detail.session.startedAt)} · 用时 {Math.floor(detail.session.elapsedSeconds / 60)} 分钟</p>{detail.session.hasFullAudio ? <audio controls preload="none" src={'/api/simulation-records/' + detail.session.id + '/audio'} /> : null}</div><button className="simulation-chat-trigger" onClick={() => setChatOpen(true)}>和教练讨论</button></header><section className="simulation-answer-list">{detail.answers.map((item) => <article key={item.id}><div className="simulation-answer-head"><span>模块 {item.moduleIndex + 1} · {item.moduleTitle}</span><small>{formatRecordDate(item.createdAt)} · {item.elapsedSeconds}s</small></div><h2>{item.question}</h2><p>{item.transcript || item.answer || '暂无文字回答'}</p>{item.hasAudio ? <audio controls preload="none" src={'/api/simulation-records/' + detail.session.id + '/audio/' + item.id} /> : null}</article>)}</section>{chatOpen && <div className="chat-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !chatLoading) setChatOpen(false); }}><section className="chat-modal" role="dialog" aria-modal="true"><div className="chat-modal-head"><h2>模拟复盘对话</h2><button className="chat-modal-close" onClick={() => setChatOpen(false)}>×</button></div><div className="chat-messages">{chatMessages.map((item, index) => <div className={'chat-message ' + item.role} key={index}><MarkdownContent value={item.content} /></div>)}</div><form className="chat-form" onSubmit={sendChat}><textarea value={chatInput} onChange={(event) => setChatInput(event.target.value)} placeholder="询问这场模拟中回答的改进建议…" /><button disabled={chatLoading || !chatInput.trim()}>发送</button></form></section></div>}</main>;
